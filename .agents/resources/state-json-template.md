...
Contains failsafe limits to prevent infinite loops and aggregates execution metrics.
...
| `totalDurationSeconds` | number | Cumulative duration in seconds across all agent runs (optional) |
| `totalTokens` | number | Cumulative total tokens consumed across all agent runs (optional) |
| `totalEstimatedCostUsd` | number | Cumulative estimated cost in USD across all agent runs (optional) |
...
Tracks the execution history for audit, metric tracking, and rollback purposes.
...
| `metrics` | object | Optional execution metrics for this run (see Metrics Object below) |

### Metrics Object (inside History entries)

Execution performance and cost metrics for a single agent run.

| Field | Type | Description |
|-------|------|-------------|
| `durationSeconds` | number | Execution duration in seconds for this run |
| `inputTokens` | number | Input/prompt tokens consumed by this run |
| `outputTokens` | number | Output/completion tokens generated by this run |
| `totalTokens` | number | Total tokens consumed by this run (`inputTokens` + `outputTokens`) |
| `model` | string | Model identifier used for this run (e.g., "claude-3-7-sonnet", "gpt-4o") |
| `estimatedCostUsd` | number | Estimated monetary cost in USD for this run |
