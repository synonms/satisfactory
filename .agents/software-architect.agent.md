...
- Create user stories or chores (that is for `triage`)
...
1. **Analyse** the user story/chore and acceptance criteria returned by `python -m tools.work_items get {work-item-id}`. If the command fails, report its structured error and stop; never access work-item storage directly.
...
- User story/chore work-item ID whose payload has clear acceptance criteria
