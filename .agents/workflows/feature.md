...
10. At `ready-for-user`, final approval requires an explicit human instruction. The driver records the approval in `history`, sets `state` to `done`, and runs `python -m tools.work_items change-status {work-item-id} done`.
