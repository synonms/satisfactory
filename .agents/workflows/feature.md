...
10. At `ready-for-user`, final approval requires an explicit human instruction. The driver records the approval in `history`, sets `state` to `done`, and updates the work-item lifecycle state according to `.agents/resources/work-items.md`.
