...
- One branch per work item, created by the driver before the first implementing agent runs:
  ...
  - `bugfix/{work-item-id}-{title}` for bugs
...
- Commit message format:
  ```
  [{work-item-id}][{chunk-id}] {agent-short-name}: iteration {n}
  ```
  for example `[00001-1][dotnet-backend] engineer: iteration 2`. Use `-` as the chunk id for work-item-scoped runs such as validation.
