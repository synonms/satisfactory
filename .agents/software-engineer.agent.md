...
2. `handoffs/{work-item-id}/state.json` to confirm my chunk, technology, and iteration number.
...
3. For a bug: run `python -m tools.work_items get {work-item-id}` to retrieve the work item, then read the failing reproduction test recorded in `handoffs/{work-item-id}/chunks/{chunk-id}/testlog.1.md`. Never access work-item storage directly. No specification exists for a bug; the failing test is my requirement source.
