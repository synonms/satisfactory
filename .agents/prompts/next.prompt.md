...
1. Read `handoffs/{work-item-id}/state.json`.
   ...
   - If it does not exist, locate the work item by ID using `.agents/resources/work-items.md`, determine its `type`, derive the flow (`user-story`/`chore` -> flow-1, `bug` -> flow-2, `documentation` -> flow-3), and create `state.json` at the flow's entry state using the schema in the workflow document. Apply the default budget values. Move the work item to the active lifecycle state according to the work-item resource.
...
3. Apply the pending transition from the previous run **before** routing.
   - If the expected artifact exists and is not yet reflected in `history`, write the transition now: update `state`, the chunk state, the attempt counters, `totalAgentRuns`, cumulative budget metrics (`totalDurationSeconds`, `totalTokens`, `totalEstimatedCostUsd` when available), the `latest*` artifact paths, and append the `history` entry (including the `metrics` block if runtime/token data is available).
...
Work item, type, flow, state, active chunk, attempts used against each applicable cap, total runs against budget, and cumulative execution metrics (duration, tokens, estimated cost) if recorded.
