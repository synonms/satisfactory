...
- For any `new` work item returned by `python -m tools.work_items list --status new`, start with `.agents/workflows/sdlc.md`.
...
## Skills
- **create-work-item** - Turn a free-text request or Azure DevOps ticket into a work item on the software-factory board.

## Work-item operations

Agents create, retrieve, list, and transition work items through `python -m tools.work_items`. Persistence paths and formats are private implementation details. See `.agents/resources/work-items.md` for the semantic and operation contract.
