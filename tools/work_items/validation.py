...
DEFAULT_SCHEMA_PATH = Path(__file__).parents[2] / ".agents/schemas/work-item.schema.json"
...
class WorkItemValidator:
    def __init__(self, schema_path: Path = DEFAULT_SCHEMA_PATH) -> None:
        try:
            ...
            raise WorkItemValidationError(f"Cannot load work-item schema: {error}") from error
